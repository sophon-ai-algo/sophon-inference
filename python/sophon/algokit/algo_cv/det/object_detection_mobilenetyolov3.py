...
"""The implementation of object detection mobilenetyolov3
"""
...
class ObjectDetectionMOBILENETYOLOV3(BaseEngine):
  """Construct mobilenetyolov3 detector
  """
  ...
  def __init__(self,
               ...
               nms_threshold=0.45,
               num_classes=80):
    super(ObjectDetectionMOBILENETYOLOV3, self).__init__()
    self.mode = mode
  ...
  def map_coord(self, rects, img_ori):
    ...
    for pt1, pt2, cls, prob in rects:
      ...
      pt2[1] = np.clip(int(pt2[1] / scaling_factor),\
          a_min=0, a_max=img_ori.shape[1])
      bboxes.append([pt1[0], pt1[1], pt2[0], pt2[1]])
      classes.append(cls)
      probs.append(prob)
  ...
  def postprocess(self, out, image):
    """Parse the mobilenetyolov3 output

...
#    print(FLAGS.input_dtype)
  input_shapes_dict = collections.OrderedDict()
  output_shapes_dict = collections.OrderedDict()
  for i, _ in enumerate(input_names):
    input_shapes_dict[input_names[i]] = input_shapes[i]
  for i, _ in enumerate(output_names):
    output_shapes_dict[output_names[i]] = output_shapes[i]

  split('pytorch', input_shapes_dict, save_dir, \
          FLAGS.model_path, params_path=None, \
          outputs=output_shapes_dict, dynamic=dynamic, layout='NCHW')
  print("Splitting done!\n")

  convert(save_dir, optimize=2, compare=True, target='BM1682')
  print("Compilation done!\n")

if __name__ == "__main__":
  #PARSER = argparse.ArgumentParser()
  PARSER = argparse.ArgumentParser(add_help=False)
  help_message = 'usage: python3 -m sophon.auto_split.split_pytorch [-h]' +\
                 '\n --save_dir SAVE_DIR' +\
                 '\n --model_path PTH_PATH' +\
                 '\n --dynamic DYNAMIC' +\

...
"""The implementation of general classification_mx
"""
...
class GeneralClassificationMX(BaseEngine):
  """Construct a general classifier
  """

  def __init__(self, arch, mode, xform, num_classes=1000):
    super(GeneralClassificationMX, self).__init__()
    self.mode = mode
  ...
  def mx_cls_preprocess(self, images):
    """ mxnet imagenet classification preprocess
    """
  ...
  def predict(self, images):
    if not isinstance(images, list):
      images = [images]
    prerocess_data = self.mx_cls_preprocess(images)

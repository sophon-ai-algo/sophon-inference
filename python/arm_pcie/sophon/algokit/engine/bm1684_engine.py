...
class BM1684Engine(BaseEngine):
  """Construct BM1684 engine from lib sail
  """

  def bm1684_init(self):
    """Init bm1684 inference
    """
  ...
  def __init__(self, context_path, is_dynamic, tpus, input_shapes, input_names,
    ...
    """
    super(BM1684Engine, self).__init__()
    self.is_dynamic = is_dynamic
    self.context_path = context_path
    self.tpus = int(tpus)
    self.inference = self.bm1684_init()  # init bm1684 inference
  ...
  def _sync_predict(self, inputs):
    ...
    for input_name in self.input_names:
      ...
      print("{} input shape: {}".format(input_name, tuple(inputs[input_name].shape)))
  ...
  def predict(self, inputs):
    """The inference interface of BM1684Engine

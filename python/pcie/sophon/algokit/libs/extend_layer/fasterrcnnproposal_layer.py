"""The implementation of fasterrcnn proposal layer
"""
from __future__ import division
import numpy as np
from .base_layer import BaseLayer
from ...utils.box_operation import bbox_transform_inv, nms


class FasterRcnnProposalLayer(BaseLayer):
  ...
  def __init__(self, layer_params, global_cfg):
    super(FasterRcnnProposalLayer, self).__init__()
    ...
    self._pre_nms_topn = global_cfg.get('pre_nms_topN', 300)
    self._post_nms_topn = global_cfg.get('post_nms_topN', 32)
    self._min_size = global_cfg.get('anchor_min_size', 16)
    self._nms_threshold = global_cfg.get('nms_threshold', 0.7)

  def forward(self, inputs):
    """FasterRcnnProposalLayer Algorithm:
    ...
    # take after_nms_topN (e.g. 64)
    # return the top proposals
    keep = nms(np.hstack((proposals, scores)), self._nms_threshold, 'union')
...
def ratio_enum(anchor, ratios):
  ...
  a_w = np.round(np.sqrt(size_ratios))
  a_h = np.round(a_w * ratios)
  anchors = makeanchors(a_w, a_h, x_ctr, y_ctr)  # (len(ratios), 4)
  return anchors

...
from detection.YoloX import YOLOX
import time
import signal
import os
import cv2
        ...
        while True:
            ...
            if self.upload_flage:
                ...
                for idx_temp, box in enumerate(det_box):
                    self.bmcv.rectangle(bm_image_ost, int(box[0]), int(box[1]), \
                                int((box[2]-box[0])), int((box[3]-box[1])), (255, 0, 0), 3)
                # self.bmcv.imwrite('{}/{}_result_{}.jpg'.format("/data/video/save_result",image_id,t2), bm_image_ost)

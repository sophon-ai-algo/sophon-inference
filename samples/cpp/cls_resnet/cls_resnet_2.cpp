 ...
 *
 * @param thread_id    Thread id
 * @param graph_name   Name of the model or graph
 * @param engine       Pointer to an Engine instance
...
int main(int argc, char** argv) {
  ...
  int thread_num = bmodel_paths.size();
  std::vector<std::thread> threads(thread_num);
  // load bmodel without builtin input and output tensors
  // each thread manage its input and output tensors
  for (int i = 0; i < thread_num; ++i) {
    threads[i] = std::thread(thread_load, i, &engine, bmodel_paths[i]);
  }
  for (int i = 0; i < thread_num; ++i) {
    threads[i].join();
  }
  auto graph_names = engine.get_graph_names();
  ...
  for (int i = 0; i < thread_num; ++i) {
    threads[i] = std::thread(thread_infer, i, graph_names[i], &engine,
                             input_path, loops, compare_path,
                             std::ref(status_promise[i]));
